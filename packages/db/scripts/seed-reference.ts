...
// Stable slug IDs so re-runs are idempotent and migrations referencing them
// (e.g. FK to a specific practice area) stay valid across environments.

const ibpChapters: (typeof schema.ibpChapters.$inferInsert)[] = [
...
const practiceAreas: (typeof schema.practiceAreas.$inferInsert)[] = [
...
async function main() {
  const client = db();
  console.info(`[seed] inserting ${ibpChapters.length} IBP chapters`);
  await client.insert(schema.ibpChapters).values(ibpChapters).onConflictDoNothing();
  console.info(`[seed] inserting ${practiceAreas.length} practice areas`);
  await client.insert(schema.practiceAreas).values(practiceAreas).onConflictDoNothing();
  console.info(`[seed] inserting ${jurisdictions.length} jurisdictions`);
  await client.insert(schema.jurisdictions).values(jurisdictions).onConflictDoNothing();
